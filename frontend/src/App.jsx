...
function LoginPage() {
    ...
    async function submit(e) {
        ...
        try {
            ...
            // assume response contains token in res.data.token (adjust if backend returns differently)
            const token = res.data?.token ?? res.data?.access_token ?? null;
            if (token) {
                saveToken(token);
                navigate("/dashboard");
            } else {
                // if API returns full user object with token elsewhere, show raw
                alert("Login succeeded but token not found in response. Check response shape.");
                console.log("login response", res.data);
            }
        } catch (err) {
            console.error(err);
            // fallback simple admin check (so dev can login to UI without working API)
            if (adminId === "admin" && password === "123456") {
                saveToken("dev-fallback-token");
                navigate("/dashboard");
            } else {
                alert("Invalid admin ID or password (or API unreachable). Check console for details.");
            }
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gray-100 ">
            <form className="bg-white p-8 rounded-lg shadow w-96 space-y-4 border border-cyan-400" onSubmit={submit}>
                <h2 className="text-xl font-bold text-cyan-400">Admin Login</h2>
                <input name="adminId" className="w-full bg-gray-200 p-2 rounded border border-gray-900/30" placeholder="Admin ID" />
                <input name="password" type="password" className="w-full bg-gray-200 p-2 rounded border border-gray-900/30" placeholder="Password" />
                <button
                    disabled={loading}
                ...
                <div className="text-xs text-gray-500 mt-2">Tip: use your real admin credentials. If API is down, fallback dev login: admin / 123456</div>
            </form>
        </div>
    );
}
...
function EventsList() {
    ...
    useEffect(() => {
        const fetchEvents = async () => {
            setLoading(true);
            try {
                const res = await fetch(`http://localhost:5000/events?limit=${limit}&offset=${offset}`);
...
function ItemsList() {
  ...
  useEffect(() => {
    ...
    const fetchItems = async () => {
      try {
        const res = await fetch(`http://localhost:5000/items?limit=50&offset=0`);
        const data = await res.json();
...
export function UsersList() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const res = await fetch("http://localhost:5000/users?limit=50&offset=0");
        const data = await res.json();
...
export function UserDetail() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        // Fetch items uploaded by this user
        const itemsRes = await fetch(`http://localhost:5000/items?user_id=${id}`);
